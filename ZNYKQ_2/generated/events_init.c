...
static void screen_audio_Tag_img_audio_playing_event_handler (lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    switch (code) {
    case LV_EVENT_CLICKED:
    {
        // 需要加入音频播放开始功能
        lv_obj_remove_flag(guider_ui.screen_audio_Tag_img_audio_play, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(guider_ui.screen_audio_Tag_img_audio_playing, LV_OBJ_FLAG_HIDDEN);
        break;
    }
    default:
        break;
...
void events_init_screen_audio_Tag (lv_ui *ui)
{
    ...
    lv_obj_add_event_cb(ui->screen_audio_Tag_img_audio_playing, screen_audio_Tag_img_audio_playing_event_handler, LV_EVENT_ALL, ui);
}
