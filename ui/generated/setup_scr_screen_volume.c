...
void setup_scr_screen_volume(lv_ui *ui)
{
    ...
    //Write codes screen_volume_img_bg
    ui->screen_volume_img_bg = lv_image_create(ui->screen_volume);
    lv_obj_set_pos(ui->screen_volume_img_bg, 0, 0);
    lv_obj_set_size(ui->screen_volume_img_bg, 240, 280);
    lv_obj_add_flag(ui->screen_volume_img_bg, LV_OBJ_FLAG_CLICKABLE);
    lv_image_set_src(ui->screen_volume_img_bg, &_bg_new_RGB565_240x280_tresh);
    lv_image_set_pivot(ui->screen_volume_img_bg, 50,50);
    lv_image_set_rotation(ui->screen_volume_img_bg, 0);

    //Write style for screen_volume_img_bg, Part: LV_PART_MAIN, State: LV_STATE_DEFAULT.
    lv_obj_set_style_image_recolor_opa(ui->screen_volume_img_bg, 0, LV_PART_MAIN|LV_STATE_DEFAULT);
    lv_obj_set_style_image_opa(ui->screen_volume_img_bg, 255, LV_PART_MAIN|LV_STATE_DEFAULT);
